...
import org.verizon.nifi.pulsar.producer.PublishPulsar;
import org.verizon.nifi.pulsar.producer.PublishPulsarRecord;
...
public class ConsumePulsarRecord extends AbstractPulsarConsumerProcessor<byte[]> {
    ...
    private void consumeMessages(ProcessSession session, final Consumer<byte[]> consumer, final List<Message<byte[]>> messages,
       ...
       RecordSchema schema = getSchema(readerFactory, messages.get(0), session);
          ...
          getLogger().error("Unable create a record writer to consume from the Pulsar topic");
           ...
           try {
               ...
               messages.forEach(msg ->{
                   ...
                   try {
                       RecordReader r = readerFactory.createRecordReader(session.create(), in, getLogger());
    ...
    private RecordSchema getSchema(RecordReaderFactory readerFactory, Message<byte[]> msg, ProcessSession session) {
        RecordSchema schema = null;
        InputStream in = null;

        try {
            in = new ByteArrayInputStream(msg.getValue());
            schema = readerFactory.createRecordReader(session.create(), in, getLogger()).getSchema();
        } catch (MalformedRecordException | IOException | SchemaNotFoundException e) {
           return null;
        } finally {
